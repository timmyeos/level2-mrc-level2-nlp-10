...
class BM25SparseRetrieval:
            ...
            if self.data_args.use_parasplit:
                ...
                for text in tqdm(self.contexts):
                    ...
                    for idx, data in enumerate(tmp):
                        if idx == 0 or idx % para_len != 0:
                            tmp_para += data
                        else:
                            contexts_2.append(tmp_para)
                            tmp_para = ""
                print("문단 분리 전 길이: ", len(self.contexts))
                self.contexts = contexts_2
                print("문단 분리 후 길이: ", len(self.contexts))
    ...
    def get_sparse_embedding(self) -> NoReturn:
        ...
        if os.path.isfile(bm25_path):
            ...
            for idx, text in enumerate(
                tqdm(self.contexts, desc="BM25 Sparse embedding")
            ):
                tmp = []
    ...
    def retrieve(
        ...
        if isinstance(query_or_dataset, str):
            ...
            ), "단일 Question(str 타입)은 아직 지원하지 않습니다."
